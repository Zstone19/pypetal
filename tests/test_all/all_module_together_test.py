...
class TestAll(unittest.TestCase):
    ...
    def test_file_layout(self):
        ...
        #Make sure shared javelin subdirectory exists
        ...
        for fdir, name in zip(subdirs[1:], self.line_names[1:]):
            for subdir in ['pyccf', 'pyzdcf', 'weights']:
    ...
    def test_res_keys(self):
        ...
        for i, key in enumerate(javelin_keys):
            self.assertIn(key, list(self.res['javelin_res'].keys()) )
    ...
    def test_res_values(self):
        ...
        for j in range( 3*len(self.line_names[1:]) ):
            self.assertEqual( len(self.res['javelin_res']['tophat_params'][j]), self.javelin_mc)

        self.assertIs( self.res['javelin_res']['cont_hpd'], None )
        self.assertEqual( len(self.res['javelin_res']['hpd']), 3 )
        for j in range(3):
            self.assertEqual( len(self.res['javelin_res']['hpd'][j]), 2 + 3*len(self.line_names[1:]) )
        ...
        for mod in modules:
            ...
            for i in range(len(self.line_names[1:])):
                ...
                if mod == 'pyccf':
                    ...
                    self.assertAlmostEqual( len(self.res['javelin_res']['tophat_params'][3*i])*(1 - self.res['weighting_res'][mod]['frac_rejected'][i]), len(self.res['weighting_res'][mod]['downsampled_lag_dist'][i]), places=5 )
                    ...
                    dist = self.res['javelin_res']['tophat_params'][3*i]



                self.assertListEqual( list(self.res['weighting_res']['pyccf']['CCCD'][i]), list(self.res['pyccf_res'][i]['CCCD_lags']) )
                self.assertListEqual( list(self.res['weighting_res']['javelin']['lag_dist'][i]), list(self.res['javelin_res']['tophat_params'][3*i]) )
    ...
    def test_individual_files_content(self):
        ...
        #Javelin
            ...
            #Chains
        chain_res = np.loadtxt( '.tmp/javelin/chain_rmap.txt', unpack=True )
        file_tophat = [ chain_res[i] for i in range(2, len(chain_res))]

        self.assertListEqual( list(np.exp(chain_res[0])), list(self.res['javelin_res']['sigma']) )
        self.assertListEqual( list(np.exp(chain_res[1])), list(self.res['javelin_res']['tau']) )
        for j in range(3*len(self.line_names[1:])):
            self.assertListEqual( file_tophat[j].tolist(), self.res['javelin_res']['tophat_params'][j].tolist() )
        ...
        for j in range(self.javelin_mc):
            self.assertAlmostEqual( np.exp(chain_res[0][j]), drw_sig, places=5 )
            self.assertAlmostEqual( np.exp(chain_res[1][j]), drw_tau, places=5 )


            #Light curve files
        file_cont = get_data('.tmp/javelin/cont_lcfile.dat')
        file_tot = get_data('.tmp/javelin/tot_lcfile.dat')

                #Continuum
        file_ydat = file_cont.mlist[0] + file_cont.blist[0]
        res_ydat = self.res['javelin_res']['cont_dat'].mlist[0] + self.res['javelin_res']['cont_dat'].blist[0]
        self.assertEqual(file_cont.nlc, self.res['javelin_res']['cont_dat'].nlc)
        ...
        for j in range(len(self.filenames[1:])):
            for k in range(len(file_tot.jlist[j])):
                ...
                res_ydat = self.res['javelin_res']['tot_dat'].mlist[j][k] + self.res['javelin_res']['tot_dat'].blist[j]
        ...
        for i, name in enumerate(self.line_names[1:]):
            ...
            for j in range(len(lag_dist_ds)):
                self.assertIn( lag_dist_ds[j], self.res['javelin_res']['tophat_params'][3*i].tolist() )

            for j in range(len(cccd_ds)):
                self.assertIn( cccd_ds[j], self.res['pyccf_res'][i]['CCCD_lags'].tolist() )
