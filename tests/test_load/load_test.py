...
class TestLoad(unittest.TestCase):
    ...
    def test_match(self):
        ...
        for i in range(len(self.filenames[1:])):
            ...
            load_keys = list(self.loaded_res['javelin_res'][i].keys())

            for key in jav_keys:
                self.assertIn(key, load_keys)

            self.assertEqual( self.line_names[i+1], self.loaded_res['javelin_res'][i]['name'] )
            ...
            self.assertListEqual(yfit.tolist(), self.loaded_res['javelin_res'][i]['fit_y'].tolist() )
            self.assertListEqual(yerrfit.tolist(), self.loaded_res['javelin_res'][i]['fit_yerr'].tolist() )
